...
module Searchable
  ...
  class_methods do
    ...
    def search(params: {}, exact_filters: [], text_fields: [])
      ...
      if exact_conditions.present?
        results = results.where(exact_conditions)
      end
      ...
      if query_term.present? && text_fields.any?
        ...
        sql_fragments = text_fields.map do |field|
          "LOWER(#{self.connection.quote_column_name(field)}) LIKE ?"
        end.join(" OR ")
        ...
        results = results.where(*sql_arguments)
      end
